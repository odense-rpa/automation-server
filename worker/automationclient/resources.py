...
def acquire_resource(fqdn: str,name: str,capabilities: str):
    try:
        ...
        # TODO: Implement release_resource


def register_resource(fqdn: str, name: str, capabilities: str) -> dict:
    response = requests.post(f"{base_url}/", json={"fqdn": fqdn, "name": name, "capabilities": capabilities})
...
def update_resource(resource_id: int, fqdn: str, name: str, capabilities: str) -> dict:
    response = requests.put(f"{base_url}/{resource_id}", json={"fqdn": fqdn, "name": name, "capabilities": capabilities})
    response.raise_for_status()
    logger.info(f"#{name} was updated")
    return response.json()

def ping_resource(resource_id: int):
    response = requests.put(f"{base_url}/{resource_id}/ping")
    response.raise_for_status()
