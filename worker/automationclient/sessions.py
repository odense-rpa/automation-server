...
def get_pending_session(resource_id: int) -> dict:
    response = requests.get(f"{base_url}/by_resource_id/{resource_id}")
...
def update_session_status(session_id: str, status: str) -> dict:
    ...
    response = requests.put(f"{base_url}/{session_id}/status", json={"status": status})
    response.raise_for_status()
    return response.json()

def get_process(session):
    response = requests.get(f"{automationserver_url}/processes/{session['process_id']}")
    response.raise_for_status()
    return response.json()
...
def add_log_message(session_id: int, message: str, workqueue_id=None) -> dict:
    ...
    response = requests.post(f"{base_url}/{session_id}/log", json=json)
